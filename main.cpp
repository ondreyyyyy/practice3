...
void handleClient(int clientSocket, DatabaseManager& dbManager, mutex& dbMutex) {
    ...
    string response;
    {
        ...
        else if (method == "GET" && path == "/order") {response = handleGetOrders(dbManager);
        }
    ...
    close(clientSocket);
    cout << "Клиент " << clientSocket << " отключен." << endl;
}
...
int main() {
    ...
    while (true) {
        ...
        cout << "Новое подключение: " << clientIP << ":" << clientPort << ". Socket: " << clientSkt << endl;
